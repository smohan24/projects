...
s.linkInternalFilters = "javascript:,windowsitpro.com"
...
/* Traffic Driver Type */
s._affListLocation = "/js/affiliate.txt";
var affclient = new XMLHttpRequest();
affclient.open("GET", s._affListLocation, false);
affclient.send(null);
_affiliatePartners = affclient.responseText;
s._channelParameter = 'Newsletter|cid,YM_MID>YesMail Campaign|eid,code>Marketing Campaign,iid>Paid Search|pid|elq>Eloqua Campaign';
s._channelDomain = 'Social Media|bebo.com,classmates.com,delicious.com,digg.com,facebook.com,flickr.com,linkedin.com,myspace.com,stumbleupon.com,twitter.com';
...
function s_doPlugins(s) {
    ...
    s.campaign = s.getQueryParam('cid,code,eid,pid,code,iid,YM_MID,elq', ':');
    s.campaign = s.getValOnce(s.campaign, 's_campaign', 0);

    /* set campaign to variable and persist */
    var campaignPenton = s.getQueryParam('cid,code,eid,pid,code,iid,YM_MID,elq', ':');
    ...
    /* capture search keywords to prop to measure bounce rates */
    s.prop51 = "did not bounce";
    var searchKeywords = document.referrer.match(/[?&]q=([\s%'"\w\n\+\.-]*)\&/);
    if (searchKeywords) {
        var searchKeywords2 = unescape(searchKeywords[1].replace(/\+/g, " "));
        s.prop51 = unescape(searchKeywords2);
    }
    ...
    if (typeof industry !== 'undefined') {
        if (industry) {
            industryType = industry + ': ' + sub_industry;
        }
        ...
        s.eVar51 = company_name; s.eVar52 = industryType; s.eVar53 = employee_count; s.eVar54 = primary_sic; s.eVar55 = address; s.eVar56 = main_phone;
    }


    /* Copy Site Section for Standardization Purposes */
    s.eVar57 = "D=ch";

    /* Copy Page Name for Standardization Purposes */
    s.eVar58 = s.prop58 = "D=pageName";

    /* YM Track */
    s.eVar59 = s.prop59 = s.getQueryParam('YM_RID');

    /* Eloqua Track */
    s.eVar49 = s.prop49 = s.getQueryParam('elq');


    /* User Agent Capture */
    s.eVar60 = "D=User-Agent";

    /* Counter eVar */
    s.eVar61 = "+1";

    /* Set Event8 on Campaign Click */
    ...
    var url = s.linkHandler('.exe|.zip|.wav|.mp3|.mov|.mpg|.avi|.wmv|.doc|.pdf|.xls|.docx|.xlsx|.pptx', 'd');
    if (url) {
        // set variables (props,evars,linkTrackVars,linkTrackEvents)
        url = url.substring(url.lastIndexOf('/') + 1);
        s.linkTrackVars = "prop32,prop58,prop63,eVar63,events";
        s.linkTrackEvents = "event22";
        if (s.prop29) {
            s.prop32 = s.prop29 + ": Download: " + url;
        }
        s.prop63 = url;
        s.eVar63 = "D=c63";
    ...
    if (url2) {
        if (s.inFilts(url2) == -1) {
            s.linkTrackVars = "prop32,prop58,prop62,eVar62,events";
            s.linkTrackEvents = "event21";
            if (s.prop29) {
                s.prop32 = s.prop29 + ": Exit: " + url2;
            }
            s.prop62 = url2;
            s.eVar62 = "D=c62";
            s.prop58 = "Exit: " + url2;
    ...
    if (s.ActionDepthTest) {
        if (s.getActionDepth("s_depth") == 1) {
            s.channelManager('cid,eid,pid,code,iid,YM_MID,elq', ':', 's_cm', '', 's_dirL');
        ...
        s.eVar33 = "D=c52"; // persisted to eVar
        s.ActionDepthTest = false;
    }
    s.prop53 = s.getAndPersistValue(s._channel, 'gpb_tdt', 0);

    /* Copy Persisted TDT Value to eVar */
    s.eVar64 = "D=c53";
    ...
    s.prop17 = s.getTimeParting('h', '-6', currentYear); // Set hour of day
    s.prop18 = s.getTimeParting('d', '-6', currentYear); // Set Day of week
    s.prop19 = s.getTimeParting('w', '-5', currentYear); // Set Weekend / Weekday

    /* Duplicate Time Parting Values */
